...
buttonnormal = [pygame.image.load(os.path.join(os.path.dirname(__file__), 'img', 'button_active.png')).convert(),
                pygame.image.load(os.path.join(os.path.dirname(__file__), 'img', 'button_passive.png')).convert()]
buttonlaunch = [pygame.image.load(os.path.join(os.path.dirname(__file__), 'img', 'launchbutton_active.png')).convert(),
                 pygame.image.load(os.path.join(os.path.dirname(__file__), 'img', 'launchbutton_passive.png')).convert()]



class Button:
    def __init__(self, rect, text, function, active = True, button = buttonnormal):
        self.rect = rect
        self.text = text
        ...
        self.buttonpic = button
        if active:
            pygame.draw.rect(GameplayConstants.screen, Colors.black, self.strokerect)
            mousepos = pygame.mouse.get_pos()
            if rect.collidepoint(mousepos):
                buttonpic = pygame.transform.scale(button[0], (rect.width, rect.height))
            else:
                buttonpic = pygame.transform.scale(button[1], (rect.width, rect.height))
            GameplayConstants.screen.blit(buttonpic, dest=(rect.left, rect.top))
            if buttonpic != buttonlaunch:
    ...
    def update(self):
        if self.active:
            ...
            if self.rect.collidepoint(mousepos):
                buttonpic = pygame.transform.scale(self.buttonpic[0], (self.rect.width, self.rect.height))
            else:
                buttonpic = pygame.transform.scale(self.buttonpic[1], (self.rect.width, self.rect.height))
            GameplayConstants.screen.blit(buttonpic, dest=(self.rect.left, self.rect.top))
            if self.buttonpic != buttonlaunch:
